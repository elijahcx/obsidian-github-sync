...
test("completed conflict diagnostics return to a healthy polling state", () => {
  const d = diagnostics("idle");
  d.lastSuccessfulSyncAt = 10_000;
  d.queue.conflictPaused = false;
  d.polling.enabled = true;
  d.polling.running = true;
  const report = buildDiagnosticReport(d);
  assert.match(report, /Status: idle/);
  assert.match(report, /Conflict paused: no/);
  assert.match(report, /Remote polling: enabled/);
  assert.match(report, /Polling running: yes/);
  assert.doesNotMatch(report, /token|Authorization|\/Users\//i);
});
